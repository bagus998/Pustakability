...
import logoSLDPI from "../../imports/Logo-Baru-SLDPI-Putih.webp";
    ...
    <footer style={{ backgroundColor: footerBg }} aria-label="Footer">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-10">
          ...
          <div className="lg:col-span-1">
            ...
            <div className="inline-flex items-center p-3 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 transition-colors">
              <img src={logoSLDPI} alt="Pusat Layanan Disabilitas & Pendidikan Inklusi UB" className="h-10 sm:h-12 w-auto max-w-full object-contain" />
            </div>
          </div>

          {/* Navigation */}
          <div>
            <h3 className="text-white mb-4" style={{ fontSize: "0.875rem", fontWeight: 600 }}>{t(T.footer.navigation)}</h3>
            <ul className="flex flex-col gap-2">
              {[
                { label: t(T.footer.home),    id: "home"    as Page },
                { label: t(T.footer.catalog), id: "catalog" as Page },
              ].map((link) => (
                <li key={link.label}>
                  <button onClick={() => onNavigate(link.id)} className="text-blue-300/60 hover:text-white transition-colors" style={{ fontSize: "0.85rem" }}>
                    {link.label}
                  </button>
                </li>
              ))}
      ...
      <div className="border-t border-white/8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5 flex flex-col sm:flex-row items-center justify-between gap-3">
          <p className="text-blue-300/40 text-center sm:text-left" style={{ fontSize: "0.78rem" }}>{t(T.footer.copyright)}</p>
